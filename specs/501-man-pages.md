...
- `netfyr-show(1)` — show daemon status and interface overview
...
SEE ALSO
       netfyr(1), netfyr-query(1), netfyr-show(1), netfyr-history(1),
...
SEE ALSO
       netfyr(1), netfyr-apply(1), netfyr-query(1), netfyr-show(1),
       netfyr-history(1), netfyr-revert(1), netfyr.yaml(5),
       netfyr-examples(7), systemd(1)
```
...
SEE ALSO
       netfyr(1), netfyr-apply(1), netfyr-query(1), netfyr-show(1),
       netfyr-history(1), netfyr-revert(1), netfyr-daemon(8), netfyr.yaml(5)
```
...
├── netfyr-show.1         (auto-generated by xtask)
...
- SPEC-356 (Show CLI — provides clap definitions for show subcommand)
...
Feature: Man page generation
  ...
  Scenario: Generate all man pages
    ...
    And it contains netfyr.1, netfyr-apply.1, netfyr-query.1, netfyr-show.1, netfyr-history.1, netfyr-revert.1
  ...
  Scenario: Top-level man page lists all subcommands
    ...
    Then the DESCRIPTION section mentions apply, query, and show subcommands
  ...
  Scenario: Show man page documents the show command
    Given man pages have been generated
    When the developer views man/netfyr-show.1
    Then the OPTIONS section lists --output
    And the EXAMPLES section contains at least two usage examples
    And the EXIT STATUS section documents codes 0 and 1
    And the SEE ALSO section references netfyr(1) and netfyr-daemon(8)
