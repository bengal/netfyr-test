...
## Verification
In addition to `cargo test` and `cargo clippy`, run `make integration-test SPEC=301` to execute this story's shell integration tests. All tests must pass. This is a required verification step — the story is not complete until shell tests pass.
